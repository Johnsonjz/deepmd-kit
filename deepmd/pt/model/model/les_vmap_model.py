...
@BaseModel.register("les_vmap")
class LESVmapModel(LESEnergyModel):
    model_type = "les_vmap"
        ...
        for nk, frame_ids in frame_groups.items():
            ...
            if can_use_vmap and len(frame_ids) > 1:
                ...
                if need_force:
                    ...
                    force_group = force_group * 2 * two_pi / volume_group.view(-1, 1, 1)
                ...
                if remove_self_interaction:
                    self_corr_group = latent_charge[frame_ids].square().sum(dim=(1, 2)) / (
                        sigma * torch.sqrt(two_pi)
                    )
                    corr[frame_ids, 0] -= self_corr_group
            else:
